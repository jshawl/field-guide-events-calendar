...
function neoncrm_calendar_register_settings_link($links)
{
    $links[] =
        ...
        '">' .
        __("Settings", "neoncrm-calendar") .
        "</a>";
    return $links;
}
add_filter(
    "plugin_action_links_" .
        plugin_basename(NEONCRM_CALENDAR_DIR . "neoncrm-calendar.php"),
    "neoncrm_calendar_register_settings_link",
);
...
function neoncrm_calendar_settings_init()
{
    register_setting(
        "neoncrm_calendar",
        "neoncrm_calendar_options",
        "neoncrm_calendar_sanitize_options",
    );
...
{
    $options = get_option("neoncrm_calendar_options", []);
    $output = $options;

    if (isset($input["neoncrm_org_id"])) {
        $output["neoncrm_org_id"] = sanitize_text_field(
            $input["neoncrm_org_id"],
        );
    }
